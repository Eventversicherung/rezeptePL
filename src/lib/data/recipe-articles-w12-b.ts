...
export const W12_FACTS_B: Record<string, ArticleFacts> = {
  "recipe-makaron-z-makiem": {
    ...
    originDe:
      "Makaron z makiem ist die Rezept für gekochte Nudeln mit Mohnmasse — Butter, Zucker, oft Rosinen — und bewusst keine gerollte Hefe-Mohnrolle. [Makowiec](/de/rezepte/makowiec) bleibt Rezept für die Mohnrolle und des Wigilia-Backprojekts; der Technik-Pillar bleibt [Makowiec-Technik](/de/blog/makowiec-technik). Parallel klar getrennt von [Makaron z serem](/de/rezepte/makaron-z-serem): dort Twaróg und Butter/Zucker als Pasta+Quark-Linie, hier trägt gemahlener Mohn den Charakter. Am [Wigilia-Speiseplan](/de/blog/wigilia-speiseplan) steht Makaron z makiem oft als machbare Mohnspeise, wenn niemand noch eine Rolle führen will — der Speiseplan bleibt Menü-Artikel, hier nur Mengen und Schritte. In der Diaspora ist das der Mohn-Teller, der in unter einer Stunde fertig ist: Nudeln kochen, Masse binden, vermengen. Kein Hefeteig, kein Rollen, kein Ofengeduld-Marathon.",
    originPl:
      "Makaron z makiem to strona przepisu Rezept: ugotowany makaron z masą makową — masło, cukier, często rodzynki — świadomie nie zwijana rolada drożdżowa. [Makowiec](/pl/rezepte/makowiec) zostaje przepisem na spirali i projektu pieczenia; filar techniki zostaje przy [technice makowca](/pl/blog/makowiec-technika). Równolegle jasno osobno od [makaronu z serem](/pl/rezepte/makaron-z-serem): tam twaróg i masło/cukier jako linia pasta+twaróg, tu charakter niesie mielony mak. Przy [menu wigilijnym](/pl/blog/menu-wigilijne) bywa wykonalną potrawą makową, gdy nikt nie chce już prowadzić rolady — artykuł menu zostaje przepisem na okazji, tu tylko ilości i kroki. W diasporze to talerz maku gotowy poniżej godziny: ugotować makaron, związać masę, wymieszać. Bez drożdży, bez zwijania, bez maratonu pieca.",
    shopDe:
    ...
    serveDe:
      "Warm auf Tellern, oft mit Puderzucker, als süßer Gang oder Mohnspeise am [Wigilia](/de/blog/wigilia-speiseplan)-Tisch neben Kompot und Fisch — Menü-Artikel bleibt der Speiseplan. Abgrenzung klar: Scheiben mit Spirale = [Makowiec](/de/rezepte/makowiec); cremige Pasta mit Quark = [Makaron z serem](/de/rezepte/makaron-z-serem). Reste kalt oder kurz in Butter in der Pfanne; nicht erneut hart kochen.",
    servePl:
    ...
    mistakesDe:
      "Mit Makowiec verwechseln (Rolle statt Pasta). Mit Makaron z serem verwechseln (Quark statt Mohn). Zu trockene, sandige Masse; bitterer alter Mohn; zu viel Nudelwasser → Suppe; Speiseplan-Mengen hier duplizieren statt zu verlinken. Hefe oder Ofenschritt als Hauptgericht hier erfinden.",
    mistakesPl:
      "Mylenie z makowcem (rolada zamiast makaronu). Mylenie z makaronem z serem (twaróg zamiast maku). Za sucha, piaszczysta masa; gorzki stary mak; za dużo wody → zupa; duplikować menu zamiast linkować. Wymyślać drożdże lub piec jako fokus.",
    variantsDe:
  ...
  "recipe-szarlotka": {
    ...
    serveDe:
      "In Stücken, oft mit Puderzucker, zu Tee oder Kaffee. Als Sonntagsschluss unter [Sonntagsessen](/de/blog/sonntagsessen-polnisch). Abgrenzung auf dem Tisch: Apfel in der Pfanne = [Racuchy](/de/rezepte/racuchy-jablka); Gewürz-Honig = [Piernik](/de/rezepte/piernik); Quarkkuchen = [Sernik](/de/rezepte/sernik). Hält ein bis zwei Tage gut abgedeckt; Freezer möglich in Stücken.",
    servePl:
    ...
    mistakesDe:
      "Mit Racuchy verwechseln (Pfanne/Hefe). Mit Piernik oder Sernik Hitze, Feuchtigkeit oder Garzeit falsch einschätzen. Zu warmes Butterteig-Kneten; zu süße Äpfel ohne Säure; zu nasser Boden (Saftsee); zu früh schneiden. Generischen Apfelkuchen statt Szarlotka backen.",
    mistakesPl:
